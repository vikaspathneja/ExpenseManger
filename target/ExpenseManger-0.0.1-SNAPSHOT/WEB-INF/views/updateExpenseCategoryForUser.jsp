...
document.getElementById("expensecategoryid").value=strOptions;
				...
				<form:form method="post" modelAttribute="expensecatpojo" 
					...
					<div class="form-group">
						<select class="form-control" id="myitem" onchange="GetOptions();">
							...
							<c:forEach items="${expenses_cat_list}" var="expcatobj">
								<option value="${expcatobj.expenseCategoryId}">${expcatobj.expenseType}</option>
							</c:forEach>
						</select>
						<form:hidden id="expensecategoryid" path="expenseCategoryId"/>
						<span id="item_result" style="color: red;"></span>
					</div>
					
					<div class="form-group">
						<form:input tabindex="2" path="expenseType"
							id="expensetype" class="form-control"
							placeholder="Enter New Expense Type" value="" />
					</div>

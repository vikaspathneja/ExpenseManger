				...
				<div id="test_div">
					<table class="table table-striped table-hover table-users"
						...
						<thead>
							...
							<form:form modelAttribute="expenses_list">
								<c:forEach items="${expenses_list}" var="list" varStatus="status">
									<tr>
										<td>${list.expenseId}</td>
										<td>${list.expenseAmount}</td>
										<td>${list.expenseDate}</td>
										<td>${list.expenseCategoryId}</td>
										<td>${list.expenseDescription}</td>
										<td>${list.userId}</td>
										<td><a class="btn mini blue-stripe" 
										href="<c:url value='expense/editexpense/${expenses_list[status.index].expenseId}' />"
										data-toggle="modal" data-target="#modalExpenseEdit">Edit</a></td>
										<td><a href="<c:url value='expense/deleteexpense/${expenses_list[status.index].userId}' />"

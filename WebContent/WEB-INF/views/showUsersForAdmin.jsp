			...
			<div class="col-sm-10">
				...
				<div id="test_div">
					<table class="table table-striped table-hover table-users"
						...
						<tbody id="tbody_id">
							<form:form modelAttribute="users_list">
								<c:forEach items="${users_list}" var="list" varStatus="status">
									<tr>
										<td>${list.userId}</td>
										<td>${list.userEmail}</td>
										<td>${list.userStatus}</td>
										<td>${list.role}</td>
										<td>${list.userMobileNumber}</td>
										<td><a class="btn mini blue-stripe"
											href="<c:url value='user/edituser/${users_list[status.index].userId}' />"
											data-toggle="modal" data-target="#modalExpenseEdit">Edit</a></td>
										<td><a
											href="<c:url value='user/deleteuser/${users_list[status.index].userId}' />"
											class="confirm-delete btn mini red-stripe" role="button">Delete</a></td>
									</tr>
								</c:forEach>
							</form:form>
						</tbody>

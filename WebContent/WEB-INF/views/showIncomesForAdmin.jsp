				...
				<div id="test_div">
					<table class="table table-striped table-hover table-users"
						...
						<thead>
							...
							<form:form modelAttribute="Incomes_list">
								<c:forEach items="${incomes_list}" var="list" varStatus="status">
									<tr>
										<td>${list.incomeId}</td>
										<td>${list.incomeAmount}</td>
										<td>${list.incomeDate}</td>
										<td>${list.incomeCategoryId}</td>
										<td>${list.incomeDescription}</td>
										<td>${list.userId}</td>
										<td><a class="btn mini blue-stripe" 
										href="<c:url value='income/editincome/${incomes_list[status.index].incomeId}' />"
										data-toggle="modal" data-target="#modalIncomeEdit">Edit</a></td>
										<td><a href="<c:url value='income/deleteincome/${incomes_list[status.index].incomeId}' />"
